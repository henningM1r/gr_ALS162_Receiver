...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def __init__(self, scaling=8, sample_rate=192000, tolerance=0.002, debug=False):
        ...
        self._subframe_lo = int(0.025 * self.sample_rate/self.scaling - self._tolerance)
        self._subframe_hi = int(0.025 * self.sample_rate/self.scaling + self._tolerance)

        # lasts roughly 1275 ms
        self._reinit_zero_lo = int(1.275*self.sample_rate/self.scaling - 4*self._tolerance)
        self._reinit_zero_hi = int(1.275*self.sample_rate/self.scaling + 4*self._tolerance)

        # after a successful zero or one-symbol has been detected
        # ignore the signal for almost the rest of the second
        self._skipframe_zero = int(0.8*self.sample_rate/self.scaling - self._tolerance)
        self._skipframe_one  = int(0.7*self.sample_rate/self.scaling - self._tolerance)
        #self._skipframe_last_sec = int(0.25*self.sample_rate/self.scaling - self._tolerance)

        print("zero_reinit_length", (self._reinit_zero_hi + self._reinit_zero_lo)/2)
    ...
    def extract_bits(self, inp_p1, inp_m1, inp_z0, inp_p2, inp_m2):
        ...
        for idx in range(len(inp_p1)):
            ...
            if _skip_count < _cur_skipframe +1:
                ...
                # increment current counters of ones per input stream, correspondingly
                if ch_p1 == 1:
                    _num_ones_p1 += 1
                ...
                if _in_sync == 0:
                    if (_num_ones_z0 >= self._reinit_zero_lo and
                        ...
                        self.message_port_pub(pmt.intern("msg_out"), pmt.intern("2"))
